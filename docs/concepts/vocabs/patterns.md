...
Each Concept must have a unique identifier that can be looked up in an application or on the web. An IRI, or [Internationalized Resource Identifier](https://en.wikipedia.org/wiki/Internationalized_Resource_Identifier), is recommended here for identifying vocabulary concepts.

IRIs are web identifiers (often expressed as URLs) that:
...
- have validation rules similar to web addresses (URLs), e.g. no spaces

An IRI for a `skos:Concept` typically follows a pattern such as:
...
* ``http://`` - it's on the web!
* ``vocabulary`` - subdomain
* ``curriculum.edu.au`` - managed or owned domain
* ``scot`` - vocabulary identifier, and
* ``15326`` - concept ID

What _is_ the concept that has ``15326`` as an identifier? You need to look it up on the web! The rationale for using `http` identifiers is so that concepts can be looked up on the web by anyone, anywhere (and by anything - humans, browsers etc.).

### Suffix patterns

The Concept ID is the trailing part of an IRI, or the _suffix_. Here we discuss advantages of different methods for minting an IRI suffix.

#### Increment suffixes

The above example IRI uses an **increment** method for generating an IRI suffix - we might expect the next IRI minted in this vocabulary to have the suffix ``15327``, and then ``15328`` and so on. This incremented number is opaque - we can't tell what the Concept is by looking at this number. This ID could be used for a `skos:Concept` in different vocabulary, but the IRI as a whole is unique.

Advantages of incremented IDs include:
- Simple to generate - in smaller vocabularies, increments can be applied manually; 
- semantically opaque: language neutral (so labels are not 'anchored' to the IRI and can be updated as needed)
- structurally opaque: increments do not indicate semantic relationships to other concepts (ensuring flexibility in structural editing)
- system generated token IDs will often follow an incremented pattern, which can be reused as IRI suffix

Disadvantages of incremented IDs include:
- evidence (but not proof) of when an ID was minted (i.e. suggests that higher numbers were added more recently). Systems and workflows relying on increments to reflect real workflows are prone to error
- deprecated concepts result in a 'series' gap, where increments are relied upon to indicate concept generation order

#### UUID suffixes

Here's an IRI with a very different kind of suffix:

> `http://vocabulary.curriculum.edu.au/crossCurriculum/f7f47140-a85e-498b-9367-0d468082fc2b`

The suffix here is a **UUID**, or a [Universally Unique Identifier](https://en.wikipedia.org/wiki/Universally_unique_identifier). Note that if we took the UUID out of context (removed the IRI stem), we could safely consider the UUID to be unique on its own terms - UUIDs are designed that way.

> 💡 A UUID can be freely generated using [online tools](https://www.uuidgenerator.net). You can also quickly generate [UUIDs for multiple rows](https://www.teachexcel.com/excel-tutorial/2395/generate-a-unique-list-of-random-numbers-with-a-simple-formula?src=yt) with a spreadsheet formula.

Advantages of UUID suffixes include:
- Always unique - even if IRI stem changes from mydomain/vocabName/ to myNewDomain/vocabOtherName, the IRI suffix can be relied on for processing current and legacy identifiers
- semantically opaque (as for **Increment** IRIs)
- structurally opaque: (as for **Increment** IRIs)
- device and workflow independent generation - can be generated at any time in any context, without risk of collision (e.g. two identical IRIs are minted by parallel workflows)

Disadvantages of UUID suffixes include:
- Quote-ability - human users cannot easily refer to UUIDs in unstructured communications, where a well-known code system may be in use; likewise, machine-users must resolve the IRI to get any information
- Character length - a UI challenge for some information systems that usually render shorter ID 

> 💡 Familiar, shorter codes from legacy data may be retained in the `skos:notation` property in addition to using a very opaque pattern for IRIs. If the skos:notation values are unique within a vocabulary, a server can be configured to accept an alternative IRI request, e.g. `myvocab/[UUID]` = `myvocab/[skos:notation]`

#### Preferred Label suffixes

A third method for constructing a Concept ID is to base the ID on whatever `skos:prefLabel` has been chosen. This makes the IRI itself readable and understandable by humans, for example:

> `https://data.idnau.org/pid/vocab/org-indigeneity/run-by-indigenous-persons`

... where:
- ``org-indigeneity`` is the ID for the vocabulary, and
- ``run-by-indigenous-persons`` is the ID for the concept, and
- "Run by Indigenous persons" is the `skos:prefLabel`
...
> 💡 Principle: _Concept identifiers should be opaque and persistent_.
